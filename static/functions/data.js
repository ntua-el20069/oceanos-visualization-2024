...
class Data {
    constructor(id, definition, slider = true, max = 100, min = 0, step = 0.1, value = (min + max) / 2, error = noError, limit = 0.10*min + 0.90*max) { // default initializations
        this.id = id;
...
allData = [                       // CAUTION!  The data.id should match the fieldname from fieldnames list
    ...
    new Data("speed", "Speed (kn)", slider = true, max = 25) , 
    new Data('rpm', 'RPM (rpm)', slider = false, max = Infinity),
    new Data("motor_tempMotor", "Motor (°C)", slider = true, max = 75) ,

    
    new Data('miles_lap', 'Miles / Lap', slider = false, max = Infinity),
    new Data("motor_current", "Motor (Amp)", slider = true, max = 250) , 
    new Data("charge", "Charge (%)", slider = true, max = 100) , 
    new Data("motor_tempMosfet", "Mosfet (°C)", slider = true, max = 90) ,

    //new Data('millis', 'Millis', slider = false, max = Infinity) ,   
    new Data('miles', 'Miles', slider = false, max = Infinity),
    new Data("battery_temperature", "Battery (°C)", slider = true, max = 50) , 
